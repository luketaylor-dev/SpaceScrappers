...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (enableZoom)
        {
            bool zoomPressed = zoomAction.WasPressedThisFrame();
            bool zoomReleased = zoomAction.WasReleasedThisFrame();
        ...
        if (enableCrouch)
        {
            ...
            bool crouchReleased = crouchAction.WasReleasedThisFrame();

            if(crouchPressed && !holdToCrouch)
            {
                Crouch();
            }
            ...
            else if(crouchReleased && holdToCrouch)
            {
                isCrouched = true;
                Crouch();
            }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && sprintAction.IsPressed() && sprintRemaining > 0f && !isSprintCooldown)
            {
